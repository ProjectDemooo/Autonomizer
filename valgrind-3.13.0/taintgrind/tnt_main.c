...
OSet *dumpSet;
static Word dumpSetCmp(void *key, void *elem)
{
    char *str1 = (char *) key;
    char *str2 = (char *) elem;

    return VG_(strcmp)(str1, str2);
}
...
static void dumpCallStk()
{
    ...
    n_ips = VG_(get_StackTrace)(tid, ips, 100, NULL, NULL, 0);

    for (i=0; i<n_ips; i++) {
	InlIPCursor *iipc = VG_(new_IIPC)(ips[i]);

	ptr = VG_(describe_IP)(ips[i], iipc);
...
void TNT_(emit_insn) (
   ...
   if (VG_(OSetGen_Lookup)(dumpSet, fnname))
     return;

   char *ptr = 
	VG_(OSetGen_AllocNode)(dumpSet, VG_(strlen)(fnname));

   VG_(strcpy)(ptr, fnname);

   VG_(OSetGen_Insert)(dumpSet, ptr);
